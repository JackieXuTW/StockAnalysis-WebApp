...
public abstract class BeanOTTZ_test {
    ...
    protected String getYahooFinaceUrl_test() throws Exception{
    ...
    public String getConvertSymbol() throws Exception{
        ...
        String sql = "select Symbol, StyleN from [dbo].[StockNameInd] where Symbol= " + "'"+testStockSymbol+ "'";
        ...
        if(style.equals("1"))
            {
               converSymbol = converSymbol.concat( testStockSymbol + ".TW"); //上市股結尾是.TW
            }
            else
            {
                converSymbol = converSymbol.concat( testStockSymbol + ".TWO"); //上櫃股結尾是.TWO
            }
